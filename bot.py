...
# Minimal Keep Alive Ping Every 2 Minutes
async def keep_alive():
    app_uri = os.environ.get("APP_URI", None)
    if not app_uri:
        print("⚠️ APP_URI not set. Skipping keep-alive ping.")
        return
    while True:
        try:
            async with ClientSession() as session:
                async with session.get(f"{app_uri}/health") as resp:
                    print(f"[KeepAlive] Pinged {app_uri}/health => {resp.status}")
...
async def start():
    ...
    app = web.AppRunner(await web_server.web_server())
    await app.setup()
